...
public class CubeConfig implements Configuration{
     ...
     * @param filename
     */
    public CubeConfig(String filename) throws FileNotFoundException {
        this.dim=3;
        this.depth = 0;
        ...
        for (String face:faces) {
            char[][] Face = new char[dim][dim];
            for (int i = 0; i < dim; i++) {
                for (int j = 0; j < dim; j++) {
                    String next = in.next();
                    Face[i][j] = next.charAt(0);
                }
            }
            this.cube.put(face, Face);
        }
...
//                System.out.println("\n\n\n"+face+" moved "+Character.toString(d));
//                System.out.println(newCube);
//                System.out.println("valid: "+newCube.isValid());
//                System.out.println("goal: "+newCube.isGoal());
            }
        }
        return successors;
    }
    ...
    public boolean isValid(){
        if (this.depth == 27){//to prevent the program from trying more than 26 moves
            return false;
        }
        int L = moves.size();
        ...
        if (L>=2) {
            if (Arrays.asList(moves.get(L - 1).split(" ")).get(0).equals(Arrays.asList(moves.get(L - 2).split(" ")).get(0))) {
                if (Arrays.asList(moves.get(L - 1).split(" ")).get(2).equals(oppMoves.get((Arrays.asList(moves.get(L - 2).split(" ")).get(2))))) {
                    return false;
                }
            }
        }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        //top
        //bottom
        CubeConfig c = new CubeConfig("testFiles/test6");
        System.out.println(c+"\n\n\n");
        for (String face:c.faces){
            for (char d:c.directions){
                CubeConfig newCube = new CubeConfig("testFiles/test6");
                newCube.move(face, d);
                newCube.moves.add(face+" moved "+Character.toString(d));
                System.out.println("\n\n\n"+face+" moved "+Character.toString(d));
                System.out.println(newCube);
